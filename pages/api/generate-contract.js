...
export default async function handler(req, res) {
  ...
  try {
    ...
    const addPage = () => pdfDoc.addPage([595, 842])
    ...
    drawText(page1, '프리랜서 인플루언서와 기업간 업무계약서', 100, y, 14)
    y -= 40
    ...
    drawText(page1, '제 4조 (계약기간)', 50, y, 11)
    y -= 20
    drawText(page1, `본 계약은 ${startDate}부터 ${endDate}까지로 한다.`, 50, y, 10)
    y -= 18
    drawText(page1, '을의 작업물 지적재산권 및 저작권의 활용기간은 최종 납품일로부터 365일로 한다.', 50, y, 10)
    y -= 35

    drawText(page1, '제 5조 (보수)', 50, y, 11)
    y -= 20
    drawText(page1, `"을"의 보수는 촬영일 이후 3일이내 지급하며 비용은 ${reward}원으로 책정한다.`, 50, y, 10)
    y -= 18
    drawText(page1, '갑은 을에게 제5조 1항에 책정된 금액에서 프리랜서 사업소득세 3.3%를 공제한 후 보수를 지급한다.', 50, y, 10)
    y -= 25
    ...
    drawText(page1, '제 6조 (작업내용)', 50, y, 11)
    y -= 20
    drawText(page1, '- "을"은 "갑"이 요구하는 성격의 업무를 성실히 이행하기로 한다.', 60, y, 10)
    y -= 18
    drawText(page1, '- 납품한 결과물의 소유권은 "갑"에게 있으며, "을"은 상업적 용도로 활용할 수 없다.', 60, y, 10)
    y -= 18
    drawText(page1, '- "을"은 작성한 결과물의 저작권에 대해 책임을 보증한다.', 60, y, 10)
    y -= 18
    ...
    drawText(page1, '제 7조 (비밀 유지)', 50, y, 11)
    y -= 20
    ...
    drawText(page2, '제 8조 (개인정보의 보호 등)', 50, y, 11)
    y -= 20
    drawText(page2, '- 모델은 업무과정에서 회사가 시행 중인 개인정보 보호정책 및 절차를 준수하여야 한다.', 60, y, 10)
    y -= 18
    drawText(page2, '- 모델은 본 계약서에 서명하는 것은 자신의 개인정보의 처리에 대한 동의를 구성한다는 점을', 60, y, 10)
    y -= 15
    drawText(page2, '  충분히 인지하고 있다.', 60, y, 10)
    y -= 35

    drawText(page2, '제 9조 (계약의 해지)', 50, y, 11)
    y -= 20
    drawText(page2, '- 회사는 모델에게 다음 각 호의 사유가 발생한 경우 본 계약을 즉시 해지할 수 있다.', 60, y, 10)
    y -= 18
    ...
    y -= 20
    drawText(page2, '- 회사는 모델에 대한 7일 전 통지로서 본 계약을 해지할 수 있다.', 60, y, 10)
    y -= 18
    drawText(page2, '- 모델은 부득이한 사유로 해지하고자 하는 경우 1개월 전에 서면 통지 후 승인을 받아야 한다.', 60, y, 10)
    y -= 40

    drawText(page2, '위와 같이 계약을 체결하고 계약서 2통을 작성, 서명 날인 후 각각 1통씩 보관한다.', 50, y, 10)
    y -= 40

    drawText(page2, `계  약  일  자 : ${startDate}`, 50, y, 10)
    y -= 50

    drawText(page2, '(갑)', 50, y, 11)
    y -= 20
